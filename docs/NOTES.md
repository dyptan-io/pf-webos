...
- **glibc shims required**: webOS's glibc is ~2.12, predating `getauxval`/`gettid`/`sendmmsg`.
  Backfilled in `src/webos/glibc_compat_shim.c` + `build.rs`, linked via `cargo:rustc-link-arg` and
...
- **`punktfunk-core` bumped** to negotiate `VIDEO_CAP_STREAMED_AU`. Video is decrypted client-side
  per packet; the CX's SoC turned out to be **4× Cortex-A55, ARMv8.2-A** running a 32-bit
  (AArch32, `armv7-unknown-linux-gnueabi`) userland — a correction of an earlier, wrong
  "Cortex-A9/ARMv7-A" assumption in this doc, made before `/proc/cpuinfo` was actually checked.
  RustCrypto's `aes`/`ghash` crates gate their hardware backend to `target_arch = "aarch64"` only,
  so on this 32-bit target AES-128-GCM ran as software, an O(bytes) cost scaling with bitrate. An
  interim fix shipped ChaCha20-Poly1305 (`VIDEO_CAP_CHACHA20`, faster in portable software, no
  hardware dependency) but that's since been superseded: `punktfunk-core` v0.19.1+ exposes a
  generic `crypto::register_aes128gcm_backend` extension point, and this client's
  `src/webos/aes.rs` registers a backend driven by the ARMv8 Crypto Extensions
...
- **Vendor decode-thread renice** (`spawn_vendor_decode_thread_renicer`): NDL's internal GStreamer
  ...
  registry — real contention on this SoC's **4 Cortex-A55 cores** (`/proc/cpuinfo`-confirmed; an
  earlier `nproc`-based "3 cores" figure here was wrong). Renicing them to -10 fixed NDL choppiness
  above 1080p outright. Matched by `:src` suffix.
...
- **The burst is 1 Gbps / 3 s, not 3 Gbps / 5 s.** The filler is decrypted on the same SoC
  that runs the UI thread, so an unbounded firehose starves the app for the whole
...
- **The probe connect must advertise the same cipher a real session negotiates** (plain
  AES-128-GCM — see the hardware AES backend above). `punktfunk-core` increments its
  `bytes_received`/`packets_received` counters *after* AEAD decrypt (`session.rs`, past the
  `Err(_) => continue // undecryptable noise` guard), so the whole measurement is bounded by
  decrypt throughput. A probe that negotiated a different cipher would report a ceiling this
  CPU cannot reach (or can exceed) with the cipher an actual stream uses — a number no session
  could ever deliver.
...
Everything above this line was originally written against an **LG CX, webOS 5.6**, and
assumed a Cortex-A9/ARMv7-A, 3-core chip — since corrected (see the hardware AES backend
entry above): the real CX SoC is **4× Cortex-A55, ARMv8.2-A**, just running a 32-bit
userland. The device now under test is a different model still, and some load-bearing
assumptions in these notes do not carry over:

| | CX (these notes, corrected) | G5 (`OLED65G58LW`, measured 2026-07-25) |
|---|---|---|
| webOS | 5.6 | **10.3.1** (Rockhopper, kernel 5.4.268) |
| CPU | 4× Cortex-A55, ARMv8.2-A | **ARMv8-A `aarch64` kernel**, `Features: aes pmull sha1 sha2 crc32 asimddp` |
| Cores | 4 | **2** (`/sys/devices/system/cpu/online` = `0-1`), max 1.4 GHz |
| Userland | 32-bit | 32-bit (`webos_imagename: lib32-starfish-global-secured`) |
| RAM | — | 2.5 GB (1.9 GB visible) |

Consequences worth being explicit about:

- **The AES argument for ChaCha20 no longer holds, on either device.** Both chips have
  ARMv8 Crypto Extensions; RustCrypto's `aes` crate only has intrinsics for `aarch64`, but
  this client no longer depends on that crate's hardware path — `src/webos/aes.rs` +
  `src/webos/aes_gcm_arm.c` drive the Crypto Extensions directly via `<arm_neon.h>`
  intrinsics, which work in a 32-bit (AArch32) userland same as an `aarch64` one. Confirmed
  on the CX (~12% CPU win over the ChaCha20 fallback); the mechanism is identical on the G5,
  but that has not been re-measured on this specific unit yet.
- **`-C target-cpu=cortex-a9` is wrong for this hardware.** Both the CX and the G5 are
  ARMv8.2-class cores (`asimddp`, `lrcpc`, `fphp`), not the in-order ARMv7 core with VFP3
  the current target-cpu assumes. Retuning would likely buy real codegen improvement, but
  would need re-verifying it doesn't regress either device — left alone here.
- **Two cores on the G5, not four.** The vendor-decode-thread renicer (`:src` pad tasks)
  matters *more* here, not less: there are fewer cores to absorb contention.
