...
- Cross target `armv7-unknown-linux-gnueabi` (tier-2) + webosbrew toolchain. Linux-aarch64-only; CI runs native, dev uses Docker.
- `.cargo/config.toml` wires linker to `scripts/cc-shim.sh` (passes `--sysroot` explicitly).
- **Soft-float was the single biggest perf fix** (~300ms → ~30ms per render). The non-`hf` target spec disables hardware FP codegen even though VFP3/NEON exist. Fix: `target-feature=+neon,+vfp3,-soft-float` + `target-cpu=cortex-a73` in `.cargo/config.toml`. This changes *codegen* only, not FFI ABI.
- **glibc shims required** (`src/glibc_compat_shim.c`): webOS glibc ~2.12 predates `getauxval`/`gettid`/`sendmmsg`. Linked via `cargo:rustc-link-arg` and **must land AFTER libstd** (single-pass linker drops `link-lib=static` too early).
...
Hybrid software/GPU design: `tiny_skia` rasterizes tiles, SDL2 composites. Redraw-on-change (no unconditional every-tick render). Key facts:
...
- **A held Back arrives as the EXIT key, not a long Back — don't time the hold yourself.** webOS does the long-press detection: a short Back tap is delivered as the Back key (`keycode 2097155`, no scancode), but *holding* Back fires webOS's own EXIT gesture, delivered as a discrete `SDL_SCANCODE_WEBOS_EXIT = 505` press — the held Back key itself never reaches the app (confirmed on-device: a long press logs no Back down/up at all). So "hold Back to open the dialog" can't work by timing Back events; instead poll `WEBOS_EXIT_SCANCODE` (edge-detected like the colour buttons — 505 is outside rust-sdl2's `Scancode` enum so it never surfaces in the safe event API) and open the disconnect/quit dialog on its rising edge. Short Back tap stays plain: forwarded to the host as Esc (stream) or back-nav (menu). This is exactly aurora-tv's split (`keyboard_webos.c`: EXIT→open overlay, BACK→VK_ESCAPE). Needs `KEYS_EXIT` (above) or the gesture SIGTERMs instead of delivering 505.
...
- `luna-send` **needs `ssh -tt`** (real PTY) or output is silently swallowed.
...
- **Lost packets concealed** with libopus PLC: ask `AudioGapTracker` how many precede the current packet, synthesize that many PLC frames first (decode with empty input).
- Underrun vs overrun now logged separately (`Underrun`/`Dropped`/`Resnapped`/`Queued`). Underrun sources: audio feed thread too slow, or main thread stats overlay renders every 500 ms on 2-core device.
...
Header signatures from `mariotaku/ss4s`. Load succeeds via `StarfishMediaAPIs_load` callback; LOADCOMPLETED may never arrive. Try `Ss4s` payload first on all releases (webOS 5 shape works on modern webOS), `Modern` as never-successful fallback (added analysis has proved inconclusive). No decode context handle; all NDL/Starfish calls serialized behind `NdlVideo::ffi` mutex (not thread-safe per header).

## On-device debugging

Run speed probe as CLI (`src/bin/pfprobe.rs`) with app dir's certs and pinned fingerprint (hex). Read `/proc/net/dev` and `/proc/net/snmp` around burst to verify link/socket-buffer drops.

## Known gaps

- Magic Remote pointer during stream: not forwarded to host (menu-only).
