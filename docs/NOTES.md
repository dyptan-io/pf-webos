...
- **Two independent cursors** — webOS draws local cursor; host draws second over network. Hiding the local one during a stream needs **both** layers: `show_cursor(false)` for SDL's own cursor object, and `platform::webos::cursor::set_visible(false)` (`SDL_webOSCursorVisibility` → `wl_webos_input_manager.set_cursor_visibility`) for the compositor's pointer, which webOS re-shows on pointer activity. `SDL_ShowCursor` alone can't fix that: it only reaches the backend when its cached `cursor_shown` flips, and `is_cursor_showing()` returns that same cache — so repeat calls (and any guard built on the query) are silent no-ops. The compositor call has no cache, so `stream.rs` re-asserts it on `MouseMotion`, rate-limited by `CURSOR_REHIDE_INTERVAL`. `mouse.rs` scales motion by `SENSITIVITY` 0.55.
